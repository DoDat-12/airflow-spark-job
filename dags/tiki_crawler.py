...
def get_product_data():
    ...
    engine = create_engine(
        "postgresql://postgres:joshuamellody@host.docker.internal:5432/test"
    )
    df_product.to_sql("product_data", engine, index=False, if_exists="replace")


def get_data():
    """Crawl product information from Tiki
    Combines get_product_id and get_product_data
    """
    ...
    df.to_parquet("/usr/local/spark/output/df.parquet.gzip",
                  compression="gzip")
